  ...
  return (
    <Box sx={{ ...props.sx }}>
      <Box
        border={2}
        borderColor={`${
          props.showError ? errorColor : isFocused ? successColor : "#79747E"
        }`}
        borderRadius="5px"
        onFocus={props.onFocus} // this props receive a function that runs when the form is being focused
        onBlur={props.onBlur} //function to run when the field is no longer focused
        sx={{
          ...
          "&:hover": {
            borderColor: `${
              props.showError ? errorColor : isFocused?successColor:"#1C1B1F" //if already interacted and input is error hover is red
            }`,
          },
        }}
      >
